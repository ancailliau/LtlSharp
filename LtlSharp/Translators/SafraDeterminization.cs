...
namespace LtlSharp.Translators
{
    ...
    /// </summary>
    public static class SafraDeterminization
    {   
        /// <summary>
        ...
        public static RabinAutomaton<AutomatonNode> Determinize (this BuchiAutomaton<AutomatonNode> ba)
        {
            var Transitions = new Dictionary<SafraTree, List<SafraTransition>> ();
            // For more details about the algorithm, check "Determinization of Büchi-Automata" by Markus Roggenbach

            var initial = new SafraTree (new [] { ba.InitialNode }, ba);
        ...
        static string PrintTree (SafraTree t, int indent = 0)
        {
            return new String ('-', indent) 
                + t.Id 
                + " - " 
                + string.Join (",", t.MacroState.Select (x => x.Name)) 
                + " - " 
                + (t.Mark ? "!" : "")
                + "\n" 
    ...
    /// </summary>
    internal class SafraTransition
    {
