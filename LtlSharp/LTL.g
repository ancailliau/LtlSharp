...
public parse returns [Expression value]
  :  formula EOF {$value = $formula.value;}
  ;
  
formula returns [Expression value]
  :  f = binary {$value = $f.value;}
  ;

binary returns [Expression value]
...
implication returns [Expression value]
  :  a = conjunction { $value = $a.value; } ('->' b = implication { $value = new Implication ($a.value, $b.value); })?
  ;
  
conjunction returns [Expression value]
  :  a = disjunction { $value = $a.value; } ('&' b = conjunction { if ($value.GetType() == typeof(Conjunction)) { ((Conjunction) $value).Push ($b.value); } else { $value = new Conjunction ($value, $b.value); } })?
  ;
  
disjunction returns [Expression value]
  :  a = unary { $value = $a.value; } ('|' b = disjunction { if ($value.GetType() == typeof(Disjunction)) { ((Disjunction) $value).Push ($b.value); } else { $value = new Disjunction ($value, $b.value); } })?
  ;
